...
public class SORBEGenerator {
	...
	private void setTripleLabel(TripleExpr newTriple,TripleExpr oldTriple) {
		if (oldTriple.getId().isBNode())
			newTriple.setId(new Label(rdfFactory.createBNode(oldTriple.getId().stringValue()+SORBE_TRIPLE_LABEL_SUFFIXE+tripleLabelNb),
	...
	public static Label removeSORBESuffixe(Label label) {
		if (label.isBNode()) {
			return new Label(rdfFactory.createBNode(label.stringValue().split(SORBE_TRIPLE_LABEL_SUFFIXE)[0]),
													label.isGenerated());
		}
