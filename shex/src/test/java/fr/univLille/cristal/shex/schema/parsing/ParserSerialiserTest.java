...
public class ParserSerialiserTest {
	...
	private static TestResultForTestReport runTest(Resource testNode, Model manifest, PrintStream passLog, PrintStream failLog, PrintStream errorLog) throws IOException {
		...
		ShexSchema schema1 = null;
		ShexSchema schema2 = null;
		try {
			...
			schema1 = GenParser.parseSchema(schemaFile,Paths.get(SCHEMAS_DIR)); // exception possible
			Path tmp = Paths.get("/tmp/schema.json");
			ShExJSerializer.ToJson(schema1, tmp);
			schema2 = GenParser.parseSchema(tmp);
			
			if (SchemaIsomorphism.areIsomorphic(schema1, schema2)){
				passLog.println(logMessage(testCase, schema1, schema2, "PASS"));
				nbPass++;
				return new TestResultForTestReport(testName, true, null, "validation");
			} else {
				failLog.println(logMessage(testCase, schema1, schema2, "FAIL"));
			...
			errorLog.println(logMessage(testCase, schema1, schema2, "ERROR"));
